$(document).ready(function(){
  ...
  function goNextTask() {
    ...
    if (index == 0) {
      setTimeout(function() {
        $('.map__char').animate({ top: 56, left: 56 }, { duration: 1100, easing: 'linear' });
        setTimeout(function() {
      ...
      $('.map__char').animate({ top: 156, left: 674 },  { duration: 2200, easing: 'linear' });
      setTimeout(function() {
        $('.map__char').animate({ top: 156, left: 800 },  { duration: 1800, easing: 'linear' });
      }, 2200);
    } else {
      $('.nav__btn_active').removeClass('nav__btn_active');
      $('.task').addClass('hidden');
      ...
      switch(index) {
        case 1:
          $('.map__char').animate({ top: 56, left: 244 },  { duration: 2700, easing: 'linear' });
          timeout = 3700;
          break;
        case 2:
          $('.map__char').animate({ top: 156, left: 464 },  { duration: 3500, easing: 'linear' });
          setTimeout(function() {
            $('.map__char').animate({ top: 86, left: 494 },  { duration: 1100, easing: 'linear' });
          }, 3500);
          timeout = 5600;
          break;
        case 3:
          $('.map__char').animate({ top: 156, left: 524 },  { duration: 1100, easing: 'linear' });
          setTimeout(function() {
            $('.map__char').animate({ top: 156, left: 674 },  { duration: 2200, easing: 'linear' });
          }, 1100);
          setTimeout(function() {
            $('.map__char').addClass('map__char_small');
            $('.map__char').animate({ top: 6, left: 644 },  { duration: 2200, easing: 'linear' });
          }, 3300);
          timeout = 6500;
          break;
        default:
  ...
  function startWires() {
    ...
    rightWiresIndexes.map(function(item, index) {
      ...
      _rightWires.push({id: index + 1, color: wiresColors[item - 1], posX: thisWire.offset().left, posY: thisWire.offset().top, obj: thisWire});
    });
  ...
  function sendAsteroid(asteroid) {
    ...
    asteroid.animate({
      ...
      }, duration: asteroidFlyingTime}, function(){
    });
    setTimeout(function(){      
